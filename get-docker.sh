...
# The script:
#
# - Requires `root` or `` privileges to run.
...
#
# 4. run the script either as root, or using  to perform the installation.
#
...
#
#   $  sh install-docker.sh --channel test
#
...
#
#   $  sh install-docker.sh --mirror AzureChinaCloud
#
# ==============================================================================
...
do_install() {
	...
	if [ "$user" != 'root' ]; then
		if command_exists ; then
			...
			We are unable to find either "" or "su" available to make this happen.
			EOF
			exit 1
		fi
	fi
